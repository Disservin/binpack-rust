...
#[derive(Debug)]
pub struct CompressedTrainingDataFile<T: Write + Read + Seek> {
    file: T,
    read_bytes: u64,
}

impl<T: Write + Read + Seek> CompressedTrainingDataFile<T> {
    pub fn new(file: T, append: bool, create: bool) -> io::Result<Self> {
...
mod tests {
    ...
    fn create_test_file(data: &[u8]) -> NamedTempFile {
        let mut file: NamedTempFile = tempfile::NamedTempFile::new().unwrap();
    ...
    fn test_new_file_creation() {
        ...
        let file2 = CompressedTrainingDataFile::new(file, false, false);
        assert!(file2.is_ok());
    }
    ...
    fn test_read_valid_chunk() {
        ...
        let mut file = CompressedTrainingDataFile::new(temp_file, false, false).unwrap();
